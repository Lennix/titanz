...
Func writeConfigPoint($point)
	...
	IniWrite($Ini,$configProperties[$point], "x" , Round($new_x))
	IniWrite($Ini,$configProperties[$point], "y" , Round($new_y))
	...
	If $point = $configNormalEntries - 1 Then
		...
		IniWrite($Ini, "filterwindow" , "left", Round($left))
		IniWrite($Ini, "filterwindow" , "top", Round($top))
		IniWrite($Ini, "filterwindow" , "right", Round($right))
		IniWrite($Ini, "filterwindow" , "bottom", Round($bottom))
	EndIf
	...
	If $point = $configNormalEntries Then
		...
		IniWrite($Ini,"diff", "filterentrydiff",Round($filterentrydiff))
		IniWrite($Ini,"diff", "entrydiff",Round($entrydiff))
		IniWrite($Ini,"diff", "itemdiff",Round($itemdiff))
	EndIf
